    ...
    <Container padding={8}>
      <VStack>
        <Breadcrumb>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink href="#">Invest in WeFund</BreadcrumbLink>
          </BreadcrumbItem>
        </Breadcrumb>
        <Heading {...headingStyle}>
          <mark>Invest</mark> in WeFund
        </Heading>
      </VStack>
    </Container>
  );
...
function IntroductionPage(props: { onClik: () => void }) {
  return (
...
function AggreementPage(props: { onClik: () => void }) {
  const onBack = () => {};
  ...
  return (
    <Container maxW={"container.sm"}>
      <VStack>
        ...
        <Text align={"center"}>
          Before you start, please prepare your identity document and make sure
          it is valid.
          <br />
          We also require you to agree to our processing of your personal data:
        </Text>
        <Checkbox>
          I agree all condition of the{" "}
          <Link>Consent of Personal Data Processing</Link>
        </Checkbox>
        <NavigationPage
          onBack={onBack}
          onNext={onNext}
  ...
  return (
    <Container>
      <VStack>
        ...
        <Text>Select the country that issued your document</Text>
        <RadioGroup>
          <Radio mx={4} value={"passport"}>
            Passport
          </Radio>
          <Radio mx={4} value={"driving-license"}>
            Driving License
          </Radio>
          <Radio mx={4} value={"id-card"}>
            ID Card
          </Radio>
          <Radio mx={4} value={"resicende-permit"}>
            Residence Permit
          </Radio>
        </RadioGroup>
      </VStack>
      ...
      <VStack>
        ...
        <Box p={"18px"} bgColor={"#FFFFFF22"} borderRadius={"12px"}>
          <VStack alignItems={"start"}>
            ...
            <Text>
              Please make sure your information in the id should be visible
            </Text>
          </VStack>
        </Box>
        <Box
    ...
    <Container maxW={"container.md"}>
      <Stack align={"center"}>
        <Stack flexDir={"row"} spacing={0} width={"100%"}>
          {steps.map((step, i) => (
            <Stack key={i} flexDir={"column"} flex={1}>
              <HStack>
                {i !== 0 ? (
                  <Divider flex={1} orientation="horizontal"></Divider>
                ) : (
                  <Box flex={1}></Box>
                )}
                <Circle
                  ...
                  backgroundColor={i <= activeStep ? "#3BE489" : undefined}
                >
...
function FundingItem(props: FundingItemProps) {
  return (
    <Box
      backgroundColor={props.odd ? "#FFFFFF1A" : "#FFFFFF08"}
      width={"full"}
      p={4}
    >
...
function FundingApproach() {
  return (
    ...
    >
      <VStack>
        <Heading {...headingStyle}>
          Our Funding <mark>Approach</mark>
        </Heading>
        <Box borderRadius={"10px"}>
          <VStack spacing={0}>
            <FundingItem
              title="SAFT"
              description="Get to know each other and Sign Agreement"
              image="/media/Invest/saft.png"
              odd={false}
            />
            <FundingItem
              title="Fund"
              description="Prefund Wefund and Receive WFD in favorable pricing"
              image="/media/Invest/fund.png"
              odd={true}
            />
            <FundingItem
              title="Lock"
              description="Wait for the Project Launch and Lock Period"
              image="/media/Invest/lock.png"
              odd={false}
            />
            <FundingItem
              title="Release"
              description="Launching the application to the market"
              image="/media/Invest/release.png"
              odd={true}
            />
          </VStack>
        </Box>
